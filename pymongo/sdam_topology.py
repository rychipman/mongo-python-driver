...
class SDAMTopology(object):
    ...
    def __init__(self, topology_settings):
        ...
        # TODO: (sdam) consider passing sdamd address in as param
        sdamd_port = int(os.getenv('MONGO_SDAMD_PORT'))

        self._sdam_monitor = SDAMMonitor(
                                 topology=self,
                                 pool=self._create_pool_for_monitor(('localhost', sdamd_port)),
    ...
    def select_servers(self,
        ...
        with self._lock:
            ...
            if not server_descriptions:
                self._sdam_monitor.request_urgent_mode()

            while not server_descriptions:
                # No suitable servers.
                if server_timeout == 0 or now > end_time:
                    self._sdam_monitor.request_end_urgent_mode()
                    raise ServerSelectionTimeoutError(
                        self._error_message(selector))

                self._ensure_opened()
                self._sdam_monitor.request_get_latest_topology()
    ...
    def on_change_topology(self, topology_description):
        # We do no I/O holding the lock.
        with self._lock:
            self._description = topology_description
            # sync server state to new topo description
    ...
    def request_check_all(self, wait_time=5):
        ...
        with self._lock:
            ...
            # TODO: (sdam) does this wait_time need to be removed?
    ...
    def _ensure_opened(self):
        ...
        if not self._opened:
            self._opened = True
            self._update_servers()
            self._sdam_monitor.open() # TODO: (sdam) do we need to do this here?
        else:
            # Restart SDAMMonitor if we forked since the previous call.
            self._sdam_monitor.open()
    ...
    def _request_check(self, address):
        ...
        # TODO: (sdam) review the cases where this gets called
        self._sdam_monitor.request_check(address)
        server = self._servers.get(address)
    ...
    def _request_check_all(self):
        ...
        # TODO: (sdam) review the cases where this gets called
        self._sdam_monitor.request_check_all()
        for server in self._servers.values():
            server.request_check()
    ...
    def _update_servers(self):
        ...
        for address, sd in self._description.server_descriptions().items():
            if address not in self._servers:
                # _settings.monitor_class will be SDAMMockMonitor
    ...
    def _create_pool_for_monitor(self, address):
        ...
        monitor_pool_options = PoolOptions(
            ...
            # TODO: (sdam) look into how the ssl options affect SDAMMonitor
